...
dag = DAG(
    ...
    description='Run data pipeline from EC2 to Snowflake using Kinesis and Airflow',
...
print("BATHCID="+batch_id)

task_customer_landing_to_processing = BashOperator(
    task_id='customer_landing_to_processing',
    bash_command= 'aws s3 mv s3://snowdpowner/firehose/customers/landing/ s3://snowdpowner/firehose/customers/processing/{0}/ --recursive'.format(batch_id),
    dag = dag
)

task_customer_processing_to_processed = BashOperator(
    task_id='customer_processing_to_processed',
    bash_command= 'aws s3 mv s3://snowdpowner/firehose/customers/processing/{0}/ s3://snowdpowner/firehose/customers/processed/{0}/ --recursive'.format(batch_id),
    dag = dag
)

task_orders_landing_to_processing = BashOperator(
    task_id='orders_landing_to_processing',
    bash_command= 'aws s3 mv s3://snowdpowner/firehose/orders/landing/ s3://snowdpowner/firehose/orders/processing/{0}/ --recursive'.format(batch_id),
    dag = dag
)

task_orders_processing_to_processed = BashOperator(
    task_id='orders_processing_to_processed',
    bash_command= 'aws s3 mv s3://snowdpowner/firehose/orders/processing/{0}/ s3://snowdpowner/firehose/orders/processed/{0}/ --recursive'.format(batch_id),
    dag = dag
)
...
(O_ORDERKEY,O_CUSTKEY,O_ORDERSTATUS,O_ORDERDATE,O_ORDERPRIORITY,O_CLERK,O_SHIPPRIORITY,O_COMMENT,BATCH_ID) from
( select t.$1,t.$2,t.$3,t.$4,t.$5,t.$6,t.$7,t.$8,'0' from @ORDERS_RAW_STAGE t)""".format(batch_id),  
]
...
task_customer_landing_to_processing >> snowflake_customers_sql_str >> task_customer_processing_to_processed] >> snowflake_order_customers_small_transformation >> post_task